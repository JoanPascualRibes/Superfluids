...
\usepackage{enumitem}
\usepackage{siunitx}
\usepackage{mhchem} % Quimica nooooo
...
\section{Eki Parte}
%%NO CONSIGO PONER EL LATEX EN EL VISUAL, ASI QUE DE MOMENTO LO DEJO ASÍ Y DESPUES DE QuÍMICA SE SIGUE INTENTANDO 
...
\todo{Quitar este párrafo si ponemos esta sección al inicio}
...
In 1924 Satyendra Nath Bose was the first to study systems of particles with a symmetric wave function, now called bosons.  He studied their statistical behaviour and also considered the possibility of cooling down such a system to very low temperatures.  He sent his work to Einstein for translation.  Einstein recognised the value of his efforts and agreed to translate the paper to German, and further made his own contributions.
\\
...
An alternative way to model the dynamic behaviour of the system is to find equations for its `density' and `velocity', similar to the way it is done in fluid dynamics, as we will see later.  Recalling that $P(\mathbf{r}) = |\Psi_0(\mathbf{r})|^2$, it makes sense to call this the density of the condensate.  It turns out that from the Gross-Pitaevskii equation one can deduce that:
